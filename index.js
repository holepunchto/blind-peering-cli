...
async function seed (opts) {
  try {
    const key = IdEnc.decode(opts.key)
    const { autoDiscDb, serviceName, debug } = opts
    const blindPeerKey = opts.blindPeerKey ? IdEnc.decode(opts.blindPeerKey) : null
    ...
    const min = blindPeerKey
      ? 1
      : parseInt(opts.min || DEFAULT_MIN)

    const limit = blindPeerKey
      ? 1
      : parseInt(opts.limit || DEFAULT_LIMIT)
    const msTimeout = parseInt(opts.timeout || DEFAULT_TIMEOUT_SEC) * 1000
    const storage = path.normalize(opts.storage || DEFAULT_STORAGE)
    ...
    for (let i = 0; i < seedProms.length; i++) {
      ...
      prom.then(
        ([dbRes, blobsRes]) => {
          if (!dbRes) {
            processReject()
            logger.warn(`Could not request db core from blind peer ${normBlindPeerKey}`)
            return
          }
          if (dbRes.announce !== true) {
